...
router.post('/items/reorder', function(req, res, next) {
  ...
  .then(function(item) {
    ...
    return Cafe.reorder(id, item.order, +item.order + diff)
...
router.post('/items/:id', upload.single('pic') , function(req, res, next) {
  ...
  .then(function(item){
      if(req.file !== undefined){
        req.body.pic = req.file.filename;
        fs.unlink('public/images/cafe/'+item.pic,function(err){
          console.log(err);
        });
      }
